...
pub fn hstack(blocks: &[Vec<Vec<f64>>]) -> Vec<Vec<f64>> {
    ...
    let total_cols: usize = blocks.iter().map(|b| if b.is_empty() { 0 } else { b[0].len() }).sum();
    let mut out = vec![vec![0.0; total_cols]; rows];
    for i in 0..rows {
        let mut col = 0;
        for block in blocks {
            if block.is_empty() { continue; }
            for &v in &block[i] { out[i][col] = v; col += 1; }
        }
    }
    out
}
...
mod tests {
    ...
    fn test_holiday_features_exact_match() {
        ...
        let timestamps = vec![
            NaiveDate::from_ymd_opt(2020, 1, 1).unwrap().and_hms_opt(0, 0, 0).unwrap(),
            NaiveDate::from_ymd_opt(2020, 1, 15).unwrap().and_hms_opt(0, 0, 0).unwrap(),
            NaiveDate::from_ymd_opt(2020, 1, 20).unwrap().and_hms_opt(0, 0, 0).unwrap(),
        ];
    ...
    fn test_holiday_features_with_windows() {
        use chrono::NaiveDate;
        let timestamps = vec![
            NaiveDate::from_ymd_opt(2020, 1, 14).unwrap().and_hms_opt(0, 0, 0).unwrap(),  // 1 day before
            NaiveDate::from_ymd_opt(2020, 1, 15).unwrap().and_hms_opt(0, 0, 0).unwrap(),  // Holiday
            NaiveDate::from_ymd_opt(2020, 1, 16).unwrap().and_hms_opt(0, 0, 0).unwrap(),  // 1 day after
            NaiveDate::from_ymd_opt(2020, 1, 17).unwrap().and_hms_opt(0, 0, 0).unwrap(),  // 2 days after
        ];
        let holidays = vec!["2020-01-15".to_string()];

...
class TestDsColumnDatetime:
    ...
    def test_ds_column_not_nat_pandas_input(self):
        ...
        assert forecast['ds'].dtype == pl.Datetime, f"Expected Datetime, got {forecast['ds'].dtype}"
        assert forecast['ds'].null_count() == 0, f"Found {forecast['ds'].null_count()} null values in ds column"
    ...
    def test_ds_column_to_pandas_conversion(self):
        ...
        assert forecast_pd['ds'].isna().sum() == 0, \
    ...
    def test_ds_column_prophet_comparison(self):
        ...
        assert forecast_pd['ds'].iloc[0] == expected_start, \
            f"First date mismatch: {forecast_pd['ds'].iloc[0]} vs {expected_start}"
        assert forecast_pd['ds'].iloc[-1] == expected_end, \
    ...
    def test_ds_column_with_string_dates(self):
        ...
        assert forecast['ds'].dtype == pl.Datetime, f"Expected Datetime, got {forecast['ds'].dtype}"
        assert forecast['ds'].null_count() == 0, \

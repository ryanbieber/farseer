...
fn make_ds(start: &str, n: usize) -> Vec<String> {
    ...
    (0..n)
        .map(|i| (start_date + chrono::Duration::days(i as i64)).format("%Y-%m-%d").to_string())
        .collect()
}
...
fn time_scale_and_changepoints() {
    ...
    let ts: Vec<_> = ds.iter().map(|s| core::trend::parse_ds(s).unwrap()).collect();
    ...
    for row in &a { assert_eq!(row.len(), cps.len()); }
}
...
fn piecewise_linear_reduces_to_linear_when_no_delta() {
    ...
    let cps: Vec<f64> = vec![];
    let a = core::trend::changepoint_matrix(&t, &cps);
    let k = 2.0; let m = 1.0; let delta: Vec<f64> = vec![];
...
fn fourier_series_shapes() {
    ...
    let days: Vec<f64> = ts.iter().map(|dt| (*dt - t0).num_seconds() as f64 / 86_400.0).collect();
...
fn model_fit_with_yearly_seasonality() {
    ...
    assert!(fcst.yearly.is_some());

    // Check MSE is reasonably small
    let mse: f64 = fcst.yhat.iter().zip(y.iter()).map(|(yh, yt)| (yh - yt).powi(2)).sum::<f64>() / n as f64;
...
fn logistic_trend_respects_cap() {
    use core::trend::{piecewise_logistic, changepoint_matrix};
    let t = vec![0.0, 0.25, 0.5, 0.75, 1.0];
    let cap = vec![100.0; 5];
    let k = 0.5; let m = 0.5; let delta = vec![];
...
fn uncertainty_intervals_scale_with_sigma() {
    ...
    for i in 0..n {
        ...
        assert!((lower_gap - upper_gap).abs() < 1e-6, "Intervals not symmetric");
...
mod rand {
    pub fn random<T: Default>() -> f64 {
        ...
        hasher.write_usize(std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_nanos() as usize);
...
fn multiplicative_seasonality_mode() {
    ...
    m.add_seasonality("weekly_mult", 7.0, 3, None, Some("multiplicative")).unwrap();
    m.fit(&data).unwrap();
    ...
    let rmse: f64 = fcst.yhat.iter()
        ...
        / n as f64;
    let rmse = rmse.sqrt();
    assert!(rmse < 30.0, "RMSE too high for multiplicative seasonality: {}", rmse);
}
...
fn mixed_additive_and_multiplicative_seasonality() {
    ...
    let y: Vec<f64> = (0..n).map(|i| {
        ...
        let multiplicative_factor = 1.0 + 0.1 * ((i as f64 / 7.0) * 2.0 * std::f64::consts::PI).cos();
    ...
    m.add_seasonality("monthly", 30.0, 5, None, Some("additive")).unwrap();
    m.add_seasonality("weekly", 7.0, 3, None, Some("multiplicative")).unwrap();
    m.fit(&data).unwrap();
...
fn invalid_seasonality_mode_returns_error() {
    ...
    assert!(result.unwrap_err().to_string().contains("Invalid seasonality mode"));
}
...
fn seasonality_with_prior_scale() {
    ...
    m.add_seasonality("custom", 15.0, 3, Some(0.1), None).unwrap();
    m.fit(&data).unwrap();
...
fn add_custom_holidays() {
    ...
    assert!(christmas_pred > day_before, "Christmas prediction should be higher than day before");
    assert!(christmas_pred > day_after, "Christmas prediction should be higher than day after");
}
...
fn holidays_with_windows() {
    ...
    assert!(during_window_avg > outside_window_avg, 
...
fn holiday_invalid_mode_returns_error() {
    ...
    assert!(result.is_err());
    assert!(result.unwrap_err().to_string().contains("Invalid holiday mode"));
}
...
fn serialization_preserves_fitted_parameters() {
    ...
    assert!((k1 - k2).abs() < 1e-6, "k parameter doesn't match: {} vs {}", k1, k2);
    
    let m1_val = params1["m"].as_f64().unwrap();
    let m2_val = params2["m"].as_f64().unwrap();
    assert!((m1_val - m2_val).abs() < 1e-6, "m parameter doesn't match: {} vs {}", m1_val, m2_val);
    
    assert_eq!(params2["fitted"], true);
    
    // Check array lengths match (exact values may differ slightly due to JSON round-trip)
    assert_eq!(params1["delta"].as_array().unwrap().len(), params2["delta"].as_array().unwrap().len());
    assert_eq!(params1["beta"].as_array().unwrap().len(), params2["beta"].as_array().unwrap().len());
}
...
fn serialization_with_custom_components() {
    ...
    m1.add_holidays("test_holiday", vec!["2020-01-15".to_string()], None, None, None, None).unwrap();
    m1.fit(&data).unwrap();
...
fn set_seasonality_mode() {
    let m = CoreSeer::new()
        .with_seasonality_mode("multiplicative");
    assert!(m.is_ok());
    
    let m_invalid = CoreSeer::new()
        .with_seasonality_mode("invalid");
